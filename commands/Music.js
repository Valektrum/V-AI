...
const video_player = async (guild, song, show_url) => {
    ...
    if(rand == 12){
        ...
        song_queue.text_channel.send("https://tenor.com/view/anime-singing-making-a-song-gif-13451239");
    }else if(rand == 10){
...
const search_song = async(message, args, Discord) => {
    if (!args.length){
        return message.channel.send('Search for what? 😾 Give me something to search for!');
    } 
    if (!message.member.voice.channel) return message.channel.send('You need to be in a channel to execute this command!');
    const video_finder = async (query) =>{
        const video_result = await ytSearch(query);
